...
In the plot of total sulfur dioxide vs. sulphates, we introduce a third variable alcohol. To do this analysis, a new variable called alcohol.factor is created that categorizes the alcohol values into four bins. For higher alcohol content, the same sulphate levels tends to correspond to higher total sulfur dioxide levels. The trend lines for three of the bins show an increasing trend, while that for wines with alcohol in range 9-10 show a decreasing trend.
...
Giving the large difference between the 75th percentile and maximum values for residual sugars, we are limiting the data to the the lower 95 percentile. For various ranges of density, there is a positive trend in the plot of residual sugar vs. alcohol content. As the alcohol content increases, for the same residual sugars, the density value is higher. 
...
When we looka t alcohol content and residual sugars as a whole, there does not seem to be a relationship. However when we include density also in the analysis, we can see a trend. The plot indicates that for various density ranges, as the alcohol content increases, the residual sugar values tend to increase.
